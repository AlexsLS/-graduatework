...
class AdaptiveHuffman:
    ...
    def _add_new_symbol(self, symbol):
        old_nyt = self.nyt
        internal = AHNode(order=old_nyt.order, parent=old_nyt.parent)
        right_order = old_nyt.order - 1
        left_order = old_nyt.order - 2
        new_nyt = AHNode(order=left_order, parent=internal, is_nyt=True)
        leaf = AHNode(symbol=symbol, order=right_order, parent=internal)
    ...
    def _decrement_weight(self, node):
        while node is not None:
            w = node.weight
            dq = self.blocks.get(w)
            leader = None
...
def compress_with_window(input_file, output_file, window_size=4096):
    ...
    for b in data:
        ...
        if len(window) == window_size:
            old = window.popleft()
            if old in ah.nodes_by_symbol:
                ah._decrement_weight(ah.nodes_by_symbol[old])
...
def main():
    ...
    compressed_file = "compressed_sliding.huff"
    decompressed_file = "decompressed_sliding.bin"

    compress_with_window(input_file, compressed_file, window_size)
